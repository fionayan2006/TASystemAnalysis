...
for r in df['Contig']: ##goes through every row by contig
    ...
    for this_tuple in list_of_regions:
        ...
        if (((ta_start>region_start) and (region_end>ta_start)) or ((ta_end>region_start) and (region_end>ta_end))):
            ...
            if (name_of_TA_Sys == 'AbiEii'):
                AbiEii_InGI = AbiEii_InGI + 1
            if (name_of_TA_Sys == 'YjhX'):
                YjhX_InGI = YjhX_InGI + 1
    ...
    if (falls_in==False):
        ...
        if (name_of_TA_Sys == 'AbiEii'):
            AbiEii_NotInGI = AbiEii_NotInGI+1
        if (name_of_TA_Sys == 'YjhX'):
            YjhX_NotInGI = YjhX_NotInGI+1
        Name_of_chromo = r
...
total_AbiEii = AbiEii_InGI+AbiEii_NotInGI
observed_AbiEii = AbiEii_InGI/total_AbiEii*100

total_YjhX = YjhX_InGI+YjhX_NotInGI
observed_YjhX = YjhX_InGI/total_YjhX*100

total_TA = TAInGI+TANotInGI
total_TA_ExcludingOutlier = total_TA-total_CbtA-total_AbiEii-total_YjhX
total_TA_InGI_Excluding_Outlier = TAInGI - CbtA_InGI - AbiEii_InGI - YjhX_InGI

observed_non_outlier = total_TA_InGI_Excluding_Outlier/total_TA_ExcludingOutlier*100
...
print('The number of AbiEii in GI = ' + str(AbiEii_InGI))
print('The number of AbiEii NOT in GI =  ' + str(AbiEii_NotInGI))
print('Total number of AbiEii = ' + str(total_AbiEii))
print("Observed_AbiEii = "+ str(observed_AbiEii))

print('The number of YjhX in GI = ' + str(YjhX_InGI))
print('The number of YjhX NOT in GI =  ' + str(YjhX_NotInGI))
print('Total number of YjhX = ' + str(total_YjhX))
print("Observed_AbiEii = "+ str(observed_YjhX))


print('Total number of non-OutlierTA = ' + str(total_TA_ExcludingOutlier))
print('Total number of non-Outlier in GI = ' + str(total_TA_InGI_Excluding_Outlier))
print('Observed non CbtA = ' + str(observed_non_outlier))
